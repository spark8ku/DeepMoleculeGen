...
__all__ = ['Builder','Optical_RNN_Builder']
...
class Builder(object):
    ...
    def __init__(self, model_loc,ctx=mx.gpu(), gpu_id=0):
        ...
        self.ctx = ctx
        self.mdl.collect_params().initialize(mx.init.Xavier(magnitude=1), force_reinit=True,ctx=self.ctx)
...
class Optical_RNN_Builder(Builder):

    @staticmethod
    def _get_model(configs):
        return models.OpticalMolGen_RNN(get_mol_spec().num_atom_types, get_mol_spec().num_bond_types, D=2, **configs)


    def sample(self, num_samples, c,sol_adj,sol_feat,NX_sol, output_type='mol', sanitize=True, random=True,X=None,A=None,NX=None,NA=None,last_action=None):
        if len(c.shape) == 1:
            c = np.stack([c, ]*num_samples, axis=0)
            #sol_adj = np.stack([sol_adj,]*num_samples,axis=0)
            #sol_feat = np.stack([sol_feat,]*num_samples,axis=0)
            #NX_sol = np.stack([NX_sol,]*num_samples,axis=0)
        with autograd.predict_mode():
            ...
            if X is None:
                print("init")
                outputs = _decode_step(X=X, A=A, NX=NX, NA=NA, last_action=last_action, finished=finished,
            ...
            #graph_list = [] # step by step
            while not np.all(finished) and count < 200:
                def get_action(inputs):
                    ...
                    _sol_adj = nd.array(sol_adj, ctx=self.ctx, dtype='float32')
                    _sol_feat = nd.array(sol_feat, ctx=self.ctx, dtype='float32')
                    _NX_sol = nd.array(NX_sol, ctx=self.ctx, dtype='float32')
                    _X, _A_sparse, _NX, _NX_rep, _mask, _NX_cum = self.to_nd(inputs)
                    _append, _connect, _end, _h = self.mdl(_X, _A_sparse, _NX, _NX_rep, _mask, _NX_cum, _h, _sol_feat, _sol_adj,_NX_sol, _c, _NX_rep)
