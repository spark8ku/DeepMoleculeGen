...
__all__ = ['get_sol_matrix', 'get_graph_from_smiles_list', 'get_mol_from_graph', 'get_mol_from_graph_list', 'get_d']

def get_sol_matrix(smiles):
    ...
    def atom_HNums(atom):
        return np.array(one_of_k_encoding_unk(atom.GetTotalNumHs(), [0, 1, 2, 3, 4]))
    ...
    def _make_matrix(smiles_list):
        adj = _convertToAdj(smiles_list)
        features, n_f = _convertToFeatures(smiles_list)
        return adj,features, n_f
    A_sol,X_sol, NX_sol = _make_matrix(smiles)
    return A_sol, X_sol, NX_sol 
...
def process_single(smiles, k, p):
    ...
    try:
        ...
        print("process_single_fail",X_0,smiles)
    X, A = [], []
    NX, NA = [], []
    mol_ids, rep_ids, iw_ids = [], [], []
    action_0, actions = [], []
